...
function harness(options: { dryRun?: boolean; paused?: boolean; suppress?: number[]; failRead?: boolean; failComment?: number; failLinear?: boolean; failSlackRead?: boolean; applied?: string[]; malformed?: string[]; failAck?: boolean } = {}) {
  ...
  const slack = {
    ...
    commands: async (read: any) => ({ ...read, applied: options.applied ?? [], malformed: options.malformed ?? [] }),
    acknowledge: async (_channel: string, applied: string[], malformed: string[]) => { for (const _ of [...applied, ...malformed]) { if (options.failAck) throw new Error('Slack reactions.add: missing_scope'); emit('slack', 'reactions.add'); } },
...
test('applied and malformed commands are acknowledged after the ledger lock write; a failed acknowledgement does not fail the run', async () => {
  const h = harness({ applied: ['2.1'], malformed: ['2.2'] }); const result = await run('o/r', h.deps, { now: NOW });
  assert.equal(result.status, 'ok'); assert.equal(h.writes.filter(x => x === 'slack:reactions.add').length, 2);
  assert.ok(h.writes.indexOf('slack:reactions.add') > h.writes.indexOf('slack:chat.update'));
  const failing = harness({ applied: ['2.1'], failAck: true }); const tolerated = await run('o/r', failing.deps, { now: NOW });
  assert.equal(tolerated.status, 'ok'); assert.deepEqual(tolerated.errors, ['Slack command acknowledgement failed']);
  const dry = harness({ dryRun: true, applied: ['2.1'] }); await run('o/r', dry.deps, { now: NOW, dryRun: true }); assert.deepEqual(dry.writes, []);
});
