...
export function applyCommands(ledger: Ledger, replies: Reply[], maintainers: string[], bot: string, now: string): { ledger: Ledger; ignored: number; changed: boolean; applied: string[]; malformed: string[] } {
  const next = structuredClone(ledger); let ignored = 0; const applied: string[] = []; const malformed: string[] = [];
  ...
  for (const reply of unique) {
    ...
    if (!match || !repo) { ignored++; malformed.push(reply.ts); continue; }
    const target = match.groups!.target!;
    const set = next.repos[repo] ??= { prs: [], logins: [] };
    const remove = match[1] === 'unskip';
    if (target.startsWith('#')) {
      const n = Number(target.slice(1));
      if (!Number.isSafeInteger(n)) { ignored++; malformed.push(reply.ts); continue; }
